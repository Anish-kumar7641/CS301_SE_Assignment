...
def add_option(art_form_name):
    ...
    art_forms[art_form_name]['view'].append(option_name)
    art_forms[art_form_name]['buy'][option_name] = option_price
...
def view_art_form(art_form_name):
    view_options = art_forms[art_form_name]["view"]
...
def buy_art_form(art_form_name):
    buy_options = art_forms[art_form_name]["buy"]
    if len(buy_options) == 0:
        ...
        if option in buy_options:
            assert buy_options[option]==art_forms[art_form_name]["buy"][option],"inconsistent value"
...
class TestArtForms(unittest.TestCase):
    ...
    def test_choose_art_form(self):
        self.assertEqual(select_art_form(), True)
    ...
    if(inpt=='1'):
        ...
        while True:
            ...
            if choice == "1":
                if login():
                    # integration testing:
                    assert select_art_form() == True, "Execution error"
            elif choice == "2":
